...
| 资源 | 单模型4B第一阶段建议 |
|---|---|
| GPU | 2026-09-21补充：4B可先评估单张32GB卡；若原实现长样本峰值过高，再优化或提高显存。80/96GB是更宽裕方案，详见[显存与租费比较](API_VS_GPU_COST_20260921.md)，均需实测 |
| 并行度 | 先batch=1，按样本分块；第二张卡用于独立分块或另一模型时记录分片和随机种子 |
| CPU / RAM | 约16核、96–128GB内存较稳妥；用户此前110GB内存实例属于合适起点 |
| 磁盘 | 2026-09-21核验：首轮4B可从30–50GB可用空间起步；四模型各保留一份建议120–150GB；多版本和多轮原始缓存按实际增长扩容，详见第11节 |
...
当前最明确的下一步是完成开跑前的复现工程准备与差异核验，按[后续费用比较](API_VS_GPU_COST_20260921.md)选择单卡做有时间上限的运行检查；本文件不表示GPU实验已经启动。

**11. 开放权重与磁盘空间（2026-09-21补充核验）**

四个模型均公开提供可下载权重，可自行部署研究，无需购买权重。准确说是开放权重，许可证并非完全相同：Qwen3两型号为Apache 2.0；DeepSeek模型卡列MIT并注明Llama3.1底座许可证；Nemotron采用NVIDIA Open Model License并列Llama3.1 Community License。复现代码可以链接官方模型并记录版本，不将大权重上传本项目Git仓库。

下表取官方仓库全部`model-*.safetensors`文件的实际字节相加，包含文件头。GB按10^9字节，GiB按2^30字节；不含依赖环境、数据、实验结果和重复下载缓存。

| 原始模型 | 实际权重文件字节 | 大小GB | 大小GiB |
| --- | ---: | ---: | ---: |
| [Qwen3-4B](https://huggingface.co/Qwen/Qwen3-4B) | 8,044,982,000 | 8.045 | 7.492 |
| [Qwen3-14B](https://huggingface.co/Qwen/Qwen3-14B) | 29,536,665,640 | 29.537 | 27.508 |
| [DeepSeek-R1-Distill-Llama-8B](https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-8B) | 16,060,556,354 | 16.061 | 14.958 |
| [Llama-3.1-Nemotron-Nano-8B-v1](https://huggingface.co/nvidia/Llama-3.1-Nemotron-Nano-8B-v1) | 16,060,556,376 | 16.061 | 14.958 |
| **合计** | **69,702,760,370** | **69.703** | **64.916** |

大小核验依据为官方[4B文件目录](https://huggingface.co/Qwen/Qwen3-4B/tree/main)、[14B文件目录](https://huggingface.co/Qwen/Qwen3-14B/tree/main)、[DeepSeek8B文件目录](https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-8B/tree/main)、[Nemotron8Bv1文件目录](https://huggingface.co/nvidia/Llama-3.1-Nemotron-Nano-8B-v1/tree/main)。将tokenizer、配置和说明也算入，四仓库本体仍约70GB。Qwen3-4B加DeepSeek8B的权重合计约24.1GB。

实际磁盘规划建议（非运行实测上限）：

| 阶段 | 建议预留可用空间 | 包含内容 |
| --- | --- | --- |
| Mac只调用API的初步实验 | 5–10GB起步 | 脚本、轻量依赖、数据及整理后的输出；若完整保留每次长前缀请求需增加空间 |
| 仅Qwen3-4B首轮 | 30–50GB | 一份权重、环境及少量数据/结果，并留下载和临时空间 |
| Qwen3-4B加DeepSeek8B | 60–80GB | 两份权重及环境、缓存、结果余量 |
| 四模型各留一份 | 120–150GB | 约70GB模型文件，加环境、数据、输出及临时空间 |
| 多权重版本、大量消融/原始请求缓存 | 200GB以上按需扩容 | 由实际保留版本和缓存增长决定，不是首次运行必需 |

若AutoDL环境主要放系统盘，免费50GB数据盘可以作为单4B阶段的起点，但先检查已有文件和剩余容量；无需为尚未使用的全部模型提前分配磁盘。模型逐个下载、顺序实验即可，同一份权重可供多种方法复用。

官方程序主要将推理、探测概率和判分结果写为JSONL；正常KV缓存在内存/显存中使用，没有默认为所有样本保存到硬盘。重复保留全请求、多模型版本或自行保存张量会改变磁盘用量。硬盘足够只说明能存放模型，不代表RAM或GPU显存足够运行。

本次仅读取公开文件列表及小型元数据，未下载模型权重、安装环境或启动实验。
